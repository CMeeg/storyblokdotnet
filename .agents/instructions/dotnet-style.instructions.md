...
- Preserve the existing package layout: production code in `packages/StoryblokDotNet.{PackageName}` and tests in `packages/StoryblokDotNet.{PackageName}.Tests`.
...
- When adding tests, place xUnit v3 test classes under `packages/StoryblokDotNet.{PackageName}.Tests` with public test classes and `[Fact]` or `[Theory]` methods so `dotnet test` can discover them.
